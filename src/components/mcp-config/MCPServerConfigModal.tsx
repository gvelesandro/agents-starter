...
export const MCPServerConfigModal: React.FC<MCPServerConfigModalProps> = ({
  ...
  const testConnection = async () => {
    ...
    try {
      ...
      // Mock result based on URL pattern
      const isValidUrl = formData.url.includes('localhost') || formData.url.includes('valid-mcp-server');
      
      if (isValidUrl) {
        setTestResult({
          success: true,
          message: 'Connection successful! Server is responding and tools were discovered.'
        });
      } else {
        setTestResult({
          success: false,
          message: 'Connection failed: Server not reachable or invalid MCP server.'
        });
      }
    } catch (error) {
      setTestResult({
