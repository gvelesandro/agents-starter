...
vi.mock("@phosphor-icons/react", () => ({
  Robot: ({ size, className }: { size?: number; className?: string }) => (
    <span data-testid="robot-icon" data-size={size} className={className}>
      🤖
    </span>
  ),
  CaretDown: ({ size, className }: { size?: number; className?: string }) => (
    <span data-testid="caret-icon" data-size={size} className={className}>
      ⌄
    </span>
  ),
}));
...
describe("ToolInvocationCard", () => {
  ...
  it("formats page URL results correctly", () => {
    ...
    expect(
      screen.getByText(/- ~ Page URL: https:\/\/example.com/)
    ).toBeInTheDocument();
