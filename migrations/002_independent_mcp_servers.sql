...
CREATE TABLE IF NOT EXISTS mcp_servers_independent (
    ...
    auth_type TEXT NOT NULL DEFAULT 'none', -- 'none' | 'apikey' | 'basic' | 'oauth2' | 'custom'
    auth_config TEXT, -- JSON string for credentials
    is_enabled BOOLEAN NOT NULL DEFAULT 1,
    status TEXT, -- 'connected' | 'disconnected' | 'error' | 'authenticating' | 'pending_auth'
...
CREATE TRIGGER IF NOT EXISTS update_mcp_servers_independent_updated_at
    AFTER UPDATE ON mcp_servers_independent
    FOR EACH ROW
    BEGIN
        UPDATE mcp_servers_independent SET updated_at = CURRENT_TIMESTAMP WHERE id = NEW.id;
    END;
